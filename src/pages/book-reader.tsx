...
export function BookReaderPage() {
  ...
  // Load bookmarks & highlights for current book
  useEffect(() => {
    if (!token || !id) return;
  ...
  return (
    <div className={`h-screen flex flex-col ${theme.bg} ${theme.text}`}>
                ...
                {chapters.map((ch, idx) => (
                  <button key={ch.id} onClick={() => goToChapter(idx)}
                    className={`block w-full text-left text-sm py-1.5 px-2 rounded truncate ${
                      idx === chapterIndex ? 'bg-blue-100 text-blue-700 font-medium' : 'hover:bg-black/5 opacity-80 hover:opacity-100'
                    }`}>
                    {ch.title}
                  </button>
                ))}
        ...
        {showBookmarks && (
          <div className={`w-72 ${theme.sidebar} border-r ${theme.border} overflow-y-auto shrink-0 absolute sm:relative z-10 h-full`}>
            <div className="p-4">
              ...
              {bookmarks.length === 0 ? (
                <p className="text-sm opacity-50">暂无书签，点击 ☆ 添加</p>
              ) : (
              ...
              <div className="flex gap-2">
                {(['light', 'sepia', 'dark'] as const).map((t) => (
                  <button key={t} onClick={() => updateSettings({ theme: t })}
                    className={`px-3 py-1 rounded text-xs border ${settings.theme === t ? 'border-blue-500 bg-blue-50' : ''}`}>
          ...
          <div ref={contentRef} className="max-w-2xl mx-auto px-6 py-8 relative" onMouseUp={handleMouseUp}>
            {chapter ? (
              <>
                ...
                <div style={{ fontSize: `${settings.fontSize}px`, lineHeight: settings.lineHeight }} className="whitespace-pre-wrap select-text">
                  {chapter.content}
                </div>
              </>
            ) : (
      ...
      <div className={`flex items-center justify-between px-4 py-2 border-t ${theme.border} shrink-0`}>
        <button onClick={goPrev} disabled={chapterIndex === 0}
          className="text-sm opacity-70 hover:opacity-100 disabled:opacity-30">← 上一章</button>
        <div className="w-48 h-1 bg-gray-200 rounded-full overflow-hidden">
          <div className="h-full bg-blue-500 transition-all" style={{ width: `${((chapterIndex + 1) / chapters.length) * 100}%` }} />
        </div>
        <button onClick={goNext} disabled={chapterIndex === chapters.length - 1}
          className="text-sm opacity-70 hover:opacity-100 disabled:opacity-30">下一章 →</button>
      </div>
    </div>
  );
}
